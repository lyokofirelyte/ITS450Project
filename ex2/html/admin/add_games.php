...
$page_title = 'ADMIN – Add Games for Sale';
...
<form enctype="multipart/form-data" action="add_games.php" method="post" accept-charset="utf-8">
	...
	<fieldset><legend>Fill out the form to add a game for sale. All fields are required.</legend>
		<div class="field"><label for="category"><strong>Genre</strong></label><br /><select name="category"<?php if (array_key_exists('category', $add_product_errors)) echo ' class="error"'; ?>>
			...
			<!-- Security measures: the 'strip_tags' function will strip all HTML and PHP tags from a variable. No tags are needed for this type of input. The 'stripslashes' function will remove backslashes -->
		
			<div class="field"><label for="name"><strong>Title</strong></label><br /><?php
    			strip_tags(stripslashes(create_form_input('name', 'text', $add_product_errors))); ?></div>
		
			<div class="field"><label for="price"><strong>Price</strong></label><br /><?php
    			strip_tags(stripslashes(create_form_input('price', 'text', $add_product_errors))); ?> <small>Without the dollar sign.</small></div>
	
			<div class="field"><label for="stock"><strong>Initial Quantity in Stock</strong></label><br /><?php
    			strip_tags(stripslashes(create_form_input('stock', 'text', $add_product_errors))); ?></div>
			
			<div class="field"><label for="description"><strong>Description</strong></label><br /><?php strip_tags(stripslashes(create_form_input('description', 'textarea', $add_product_errors))); ?></div>
	...
	<div class="field"><input type="submit" value="Add This Game" class="button" /></div>
